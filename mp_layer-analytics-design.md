...
- **Kiến trúc ghi:** sendBeacon → beacon **chỉ INSERT bảng raw** (append-only); cron **aggregate tăng dần mỗi 15 phút** từ raw vào bảng report theo watermark `log_id` (kiến trúc report của Magento core). Dashboard chỉ đọc bảng report, số liệu trễ tối đa ~15 phút. Không message queue (extension không được giả định merchant có RabbitMQ/Redis; MySQL-backed queue cũng chỉ là 1 INSERT khác, không lợi gì).
- **Schema 3 bảng:** `mageplaza_layer_analytics_request_log` (raw track, bám spec gốc §3.2) + `..._report_popular` + `..._report_zero` (2 báo cáo độc lập, tổng hợp theo giờ cho dashboard). Không dùng mô hình cha–con/FK CASCADE.
...
│                                                                       │
│  Cron\Aggregate (0 * * * *) → đọc raw log_id > watermark, UPSERT:     │
│    ├─ ..._report_popular: mỗi (attr,value) 1 dòng/giờ, hits cộng dồn  │
│    └─ ..._report_zero:    1 dòng/URL/giờ khi result_count=0           │
│                                                                       │
│  Cron\CleanLog (0 1 * * *) → DELETE raw log cũ theo batch             │
│   (KHÔNG đụng report — report chỉ xóa khi Admin bấm Clear Report)     │
│                                                                       │
│  ADMIN  mplayer/analytics/index   (đọc DUY NHẤT 2 bảng report)        │
...
- Trả về JSON:
  ...
  - **Quy tắc multi-select:** 1 phần tử = 1 cặp (attribute, value). 5 attribute / 8 value = payload 8 phần tử = 8 dòng raw chung `request_uid` (1 câu INSERT). Giới hạn 20 phần tử/payload đếm theo cặp. Top Filters đếm từng value riêng; Zero-results 1 hit/lượt.
...
6. Exception → nuốt + ghi `var/log/mplayer_analytics.log` (logger riêng qua virtualType handler). Response: `204 No Content`, `Cache-Control: no-store`, không render layout (`ResultFactory::TYPE_RAW`).

**Chi phí mỗi beacon:** bootstrap Magento + 1 INSERT + tối đa N+1 upsert ≈ nhẹ hơn nhiều một page view; beacon chạy nền nên khách không cảm nhận được (R1, R3).

---

## 4. DB Schema (`etc/db_schema.xml` + `db_schema_whitelist.json` của Ultimate)

Mô hình **3 bảng**: 1 bảng raw track + 2 bảng report tổng hợp (popular & zero-result tách riêng vì là 2 báo cáo độc lập, schema khác hẳn nhau). Không có FK — report là dữ liệu dẫn xuất (derived), cron cộng dồn từ raw.
...
Bảng này là **nguồn chân lý** (source of truth): dùng đối soát, debug, và tái dựng lại 2 bảng report nếu cần (re-aggregate theo `request_uid`).

### 4.2. `mageplaza_layer_analytics_report_popular` — filter được dùng nhiều
...
| `hits` | int unsigned | N | số lượt, cộng dồn |
| `last_requested` | timestamp | N | lần gần nhất |

- **UNIQUE KEY** (`report_date`, `report_hour`, `store_id`, `attribute_code`, `option_value`) — khóa cho `ON DUPLICATE KEY UPDATE`, đặt thẳng trên cột thật.
- **Indexes:** (`report_date`).

### 4.3. `mageplaza_layer_analytics_report_zero` — tổ hợp filter ra 0 sản phẩm

| Cột | Kiểu | Null | Ghi chú |
|---|---|---|---|
| `report_id` | int unsigned, PK, AI | N | |
| `report_date` | date | N | ngày ghi nhận |
| `report_hour` | smallint unsigned | N | 0–23 |
| `store_id` | smallint unsigned | N | default 0 |
| `url_hash` | char(32) | N | `md5(target_url)` — khóa UNIQUE (vì `target_url` varchar(2048) quá dài để index trực tiếp) |
| `target_url` | varchar(2048) | N | URL đầy đủ — merchant đọc URL là thấy ngay tổ hợp filter nào gây zero |
| `hits` | int unsigned | N | số **lượt** ra 0 sản phẩm, cộng dồn (đếm theo `request_uid`, không nhân theo số filter) |
| `last_requested` | timestamp | N | lần gần nhất |

- **UNIQUE KEY** (`report_date`, `report_hour`, `store_id`, `url_hash`) — khóa cho `ON DUPLICATE KEY UPDATE`.
- **Indexes:** (`report_date`).

Cả 2 bảng tổng hợp ở **mức giờ**: dashboard mức ngày chỉ là `SUM(hits)` gộp 24 dòng. Bảng nhỏ (vài nghìn dòng/ngày/store) → dashboard nhanh ở mọi quy mô.

### 4.4. Resolve label động khi query (bảng popular)
...
- Đếm **Total Hits của Zero-results chính xác theo lượt** (1 lượt lọc 3 filter ra 0 kết quả = 1 hit, không phải 3) vì cron đếm theo `request_uid`.
...
- **Popular** (`..._report_popular`): `COUNT(*)` theo (`DATE(created_at)`, `HOUR(created_at)`, `store_id`, `attribute_code`, `option_value`) trên batch → bulk `INSERT ... ON DUPLICATE KEY UPDATE hits = hits + VALUES(hits), last_requested = VALUES(last_requested)`.
- **Zero-result** (`..._report_zero`): gom các `request_uid` có `result_count = 0` (1 lượt = 1 hit, không nhân theo số filter) → lấy `target_url` từ 1 dòng bất kỳ của lượt → tính `url_hash = md5(target_url)` → upsert theo (`DATE`, `HOUR`, `store_id`, `url_hash`) với `target_url`, `hits += COUNT(DISTINCT request_uid)`, `last_requested = MAX(created_at)`.
...
- **Hệ quả:** dashboard trễ tối đa ~15 phút; trang Analytics hiển thị ghi chú "Data updated at <thời điểm watermark>".
...
- `Cron/CleanLog.php`:
  ...
  - **Chỉ xóa bảng raw log** — đúng nguyên văn spec gốc §2.1. **Tuyệt đối không xóa 2 bảng report.**
...
- 2 bảng report không bao giờ bị xóa tự động. Trên trang Analytics dashboard có nút **"Clear Report Data"**:
  ...
  - Bấm nút → confirm dialog ("This will permanently delete all aggregated report data. Continue?") → `TRUNCATE` cả 2 bảng report → message thành công, redirect về dashboard.
...
### 7.3. Data providers — đọc DUY NHẤT 2 bảng report (không đụng bảng raw)

**Top Popular Filters** (`..._report_popular`):
...
SELECT r.attribute_code,
       ...
       SUM(r.hits) * 100.0 / (
           SELECT SUM(hits) FROM mageplaza_layer_analytics_report_popular
           WHERE report_date BETWEEN :from AND :to
       ) AS percentage
FROM mageplaza_layer_analytics_report_popular r
...
*(Price / Category: resolve trong Collection PHP theo logic §4.4 thay vì JOIN trực tiếp trong SQL.)*

Cột grid: Attribute Name (resolve label EAV) | Option Label (resolve động) | Total Clicks | Percentage.

**Zero-results** (`..._report_zero`):

```sql
SELECT MAX(target_url)    AS target_url,
       SUM(hits)          AS total_hits,
       MAX(last_requested) AS last_requested
FROM mageplaza_layer_analytics_report_zero
WHERE report_date BETWEEN :from AND :to
GROUP BY url_hash
ORDER BY total_hits DESC
```

Cột grid: Target URL | Total Hits | Last Requested.
...
- Lưu ý kỹ thuật: filter/sort trên cột aggregate (`total_clicks`, `total_hits`) phải map qua `addFieldToFilter` → `HAVING` trong collection custom.
...
- Endpoint track **không bao giờ** ném lỗi ra khách: mọi nhánh sai đều 204; exception ghi log riêng `var/log/mplayer_analytics.log`.
...
| `Model/Analytics/Logger.php` | Writer v1: INSERT raw append-only, chuẩn hóa hash |
| `Model/ResourceModel/RequestLog.php` (+ Collection) | Resource model bảng raw |
| `Model/ResourceModel/ReportPopular.php` (+ Collection) | Resource model bảng `..._report_popular` |
| `Model/ResourceModel/ReportZero.php` (+ Collection) | Resource model bảng `..._report_zero` |
| `Model/ResourceModel/ReportPopular/Grid/Collection.php` | SearchResult grid Top Filters (resolve label §4.4) |
| `Model/ResourceModel/ReportZero/Grid/Collection.php` | SearchResult grid Zero-results |
...
|---|---|---|
| Unit | Chuẩn hóa hash: thứ tự filter khác nhau → cùng hash; label đúng format | phpunit |
...
| Integration | Cron Aggregate: hits popular & zero đúng, chạy lặp không đếm đôi (watermark), batch cắt đúng ranh giới request_uid | phpunit integration |
...
| E2E | Lọc ra 0 sản phẩm → grid Zero-results hiện tổ hợp với Total Hits = số lượt (không nhân theo số filter) | admin grid |
| E2E | Reload cùng URL filter → KHÔNG log đúp (sessionStorage dedup) | check DB count |
...
- Số liệu dashboard trễ tối đa ~15 phút theo chu kỳ cron Aggregate (hiển thị "Data updated at" trên trang).
- Report tích lũy vĩnh viễn (không tự xóa): bảng zero-result mỗi URL/giờ là 1 dòng — sau nhiều năm bảng vẫn chỉ cỡ trăm nghìn dòng, chấp nhận được; Admin có nút Clear Report Data khi muốn làm gọn.
...
- `option_label` chốt tại thời điểm click: đổi tên option sau này sẽ thấy 2 dòng riêng trong báo cáo (chấp nhận — phản ánh đúng lịch sử).
- Số liệu report chỉ tồn tại trong cửa sổ `clean_log_days` (clean log = clean report, theo yêu cầu).
